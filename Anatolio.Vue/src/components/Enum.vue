<template>
    ...
    <span v-if="viewOnly">
        <Tag v-if="!soft" style="padding-right: 10px"
            :severity="$enums[type].find(s => s.value == modelValue)?.severity ?? 'contrast'">
            <i v-if="$enums[type].find(s => s.value == modelValue)?.icon"
                :class="$enums[type].find(s => s.value == modelValue)?.icon" style="padding-right:10px"></i>&nbsp;
            {{ $t(modelValue ?? "") }}
        </Tag>
